(() => {
  ...
  function addStyles() {
    ...
    style.textContent = `
      .rx-shared-passport-action{width:100%;display:grid;grid-template-columns:32px minmax(0,1fr) auto;align-items:center;gap:10px;border:1px solid rgba(23,128,92,.16);background:linear-gradient(135deg,#F5FBF8,#ECF8F2);color:#123B2C;border-radius:14px;padding:10px 12px;text-align:left;box-shadow:0 5px 16px rgba(23,107,73,.055);transition:background .18s ease,border-color .18s ease,box-shadow .18s ease,transform .18s ease}.rx-shared-passport-action:hover{background:linear-gradient(135deg,#F0FAF5,#E4F5EC);border-color:rgba(23,128,92,.28);box-shadow:0 8px 20px rgba(23,107,73,.09);transform:translateY(-1px)}.rx-shared-passport-action:focus-visible{outline:3px solid rgba(23,107,255,.2);outline-offset:2px}.rx-shared-passport-action:disabled{cursor:wait;opacity:.72;transform:none}.rx-shared-passport-icon{display:grid;place-items:center;width:32px;height:32px;border-radius:10px;background:#176B49;color:#fff;font-size:13px;box-shadow:0 5px 12px rgba(23,107,73,.18)}.rx-shared-passport-copy{min-width:0;display:grid;gap:2px}.rx-shared-passport-copy b{font-size:11.5px;font-weight:900;line-height:1.25;color:#123B2C}.rx-shared-passport-copy small{font-size:10.5px;font-weight:750;line-height:1.25;color:#5F7C70}.rx-shared-passport-arrow{color:#4B7E6B;font-size:17px;line-height:1;transition:transform .18s ease}.rx-shared-passport-action:hover .rx-shared-passport-arrow{transform:translateX(2px)}
  ...
  function decorateCards() {
    ...
    try {
      document.querySelectorAll('.rx-app-card, .rx-pipeline-card').forEach(card => {
        ...
        const itemLabel = `${count} item${count === 1 ? '' : 's'} shared`;
        const html = `<button class="rx-shared-passport-action" type="button" data-view-shared-passport="${safe(applicationId)}" data-shared-passport-count="${count}" aria-label="View ${itemLabel} in the candidate's shared Career Passport"><span class="rx-shared-passport-icon" aria-hidden="true">✓</span><span class="rx-shared-passport-copy"><b>View shared Career Passport</b><small>${itemLabel}</small></span><span class="rx-shared-passport-arrow" aria-hidden="true">›</span></button>`;
