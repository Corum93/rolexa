(() => {
  if (window.__rolexaDashboardMobileMenuV9) return;
  window.__rolexaDashboardMobileMenuV9 = true;
  ...
  if (!document.querySelector('link[data-rx-sidebar-premium]')) {
    ...
    sidebarTheme.href = 'dashboard-sidebar-premium.css?v=9';
  ...
  if (!document.querySelector('link[data-rx-mobile-premium-sync]')) {
    ...
    mobileTheme.href = 'mobile-premium-sync.css?v=3';
    mobileTheme.dataset.rxMobilePremiumSync = 'true';
    document.head.appendChild(mobileTheme);
  }

  const loadEnhancement = (src, marker) => {
    if (window[marker] || document.querySelector(`script[data-rx-enhancement="${marker}"]`)) return;
    window[marker] = true;
    const script = document.createElement('script');
    script.src = src;
    script.defer = true;
    script.dataset.rxEnhancement = marker;
    document.head.appendChild(script);
  };
  loadEnhancement('dashboard-mobile-menu-premium.js?v=2','__rolexaPremiumDashboardMenuRequestedV2');
  loadEnhancement('dashboard-messages-layout.js?v=3','__rolexaMessagesLayoutRequestedV3');
  ...
  const options = isCandidate
    ? [
        ...
        ['Career Passport','careerPassport'],
  ...
  options.forEach(([label,view],index) => {
    ...
    item.addEventListener('click',async () => {
      ...
      if (view === 'termsAgreement') {
        location.href = 'employer-terms.html?view=1';
        return;
      }
      if (view === 'editProfile') {
