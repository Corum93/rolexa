(() => {
  ...
  function fieldLabel(field) {
    return field.querySelector('b')?.textContent?.trim().toLowerCase() || '';
  }
  ...
  function groupPreferences() {
    ...
    section.className = 'rx-employer-preferences';
    section.innerHTML = '<h3 class="rx-employer-preferences-title">Employment preferences</h3><div class="rx-employer-preferences-grid"></div>';
    const inner = section.querySelector('.rx-employer-preferences-grid');
    fields.forEach(field => inner.appendChild(field));
  ...
  function enhanceSkills() {
    ...
    const skillsField = fields.find(field => fieldLabel(field) === 'skills');
